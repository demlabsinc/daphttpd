        ...
        if(buf[pos]=='\n'){
            ...
            switch(p_st){
                case PS_ACTION:{
                    ...
                    log_it(L_WARNING, "Input: action '%s' pos=%lu pos_kw_begin=%lu", cl_ht->action,pos,pos_kw_begin);
                    p_st=PS_URL;
                    pos_kw_begin=pos+1;
                }break;
                case PS_URL:{
                    ...
                    log_it(L_WARNING, "Input: url '%s' pos=%lu pos_kw_begin=%lu", cl_ht->url_path,pos,pos_kw_begin);
                    p_st=PS_TYPE;
                    pos_kw_begin=pos+1;
                }break;
                default:
...
cnt:switch(cl_ht->state_read){
    ...
    if(cl->buf_in_size>0){
        //log_it(L_WARNING,"Continue to process to parse input");
        goto cnt;
    }
}
